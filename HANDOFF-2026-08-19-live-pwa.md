...
## How it works
- `live_pull.py` → `private/live.json` (~25 cheap API calls: GA4 realtime + today + 28d + per-slug 7d views; Ghost members/posts; Mailchimp list/unsubs/last campaign; Donorbox 28d; Bluesky/LinkedIn + `data/social_history.json`). Every source wrapped → `{available:false, reason}`; fails loud only if ALL six fail.
...
5. **Press is NOT pulled live.** An unscoped Google News search for the phrase "Vital City" matches loosely and returned Hoodline crime-blotter items that never mention us (verified). The deep tracker queries each whitelisted outlet with `site:<domain>` — that scoping is what makes its counts trustworthy — so the app reads `DEEP.news_mentions` (kind `media`, most recent three, with outlet and date) and `live_pull.py` does no news search at all. Press is low-frequency: the newest tracked mention is often a week old.
